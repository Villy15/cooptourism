  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView (
        child: FutureBuilder(
          future: joinCooperativeRepository.getCoopApplication(widget.coopAppId),
              ...
              return FutureBuilder(
                ...
                builder: (context, snapshot) {
                  if (snapshot.hasData) {
                    ...
                    if (userData.uid != null && userData.profilePicture != null) {
        return Column(
          ...
          children: [
            ...
            Center(
              child: DisplayImage(
                path: '${userData.uid}/images/${userData.profilePicture}', 
                height: 120, 
                width: 120, 
              ...
              itemBuilder: (context, index) {
                ...
                return FutureBuilder<String>(
                  future: FirebaseStorage.instance
                          .ref('${coopApp['coopId']}/appforms/${coopApp['userUID']}/documents/$fileName')
                          .getDownloadURL(),
                  builder: ((context, snapshot) {
                    if (snapshot.hasData) {
                      final fileURL = snapshot.data;
                      final fileExtension = p.extension(fileName).toLowerCase();
                      debugPrint("$fileExtension is the file extension");
                      return InkWell(
                        onTap: () async {
                          if (fileExtension == '.pdf') {
                            ...
                            debugPrint('$path this is the path');
                            
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ViewPDFPage(path: path),
                              ),
                            );
                          }
                            ...
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => ViewImagePage(url: fileURL!),
                              ),
                            );
                          }
                          else {
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  onPressed: () async {
                    try {
                      ...
                      userData.cooperativesJoined?.add(coopApp['coopId']);
                    
                      await userRepository.updateUser(userData.uid!, userData);
                      await cooperativesRepository.addCooperativeMember(coopApp['coopId'], userData.uid!);
                      await joinCooperativeRepository.approveCoopApplication(widget.coopAppId);
                ...
                ElevatedButton(
                  onPressed: () async {
                    await joinCooperativeRepository.deleteCoopApplication(widget.coopAppId);
